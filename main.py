...
# Function to generate random values for r1 and r2
def Generate_random():
...
for n in range(num_iterations):
    ...
    for _ in range(num_particles):
        random_row = [random.uniform(bound[0], bound[1]) for bound in bounds]
        particle_velocity = [random.uniform(velocity_bounds[dim][0], velocity_bounds[dim][1]) for dim in range(len(velocity_bounds))]
...
# Plot optimal fitness over iterations
plt.subplot(2, 2, 1)
plt.plot(range(num_iterations), optimal, color='blue')
plt.title('Optimal Fitness over Iterations')
plt.xlabel('Iteration')
plt.ylabel('Optimal Fitness')

# Plot S1 over iterations
plt.subplot(2, 2, 2)
plt.plot(range(num_iterations), bestS1, color='green')
plt.title('S1 over Iterations')
plt.xlabel('Iteration')
plt.ylabel('S1')

# Plot S2 over iterations
plt.subplot(2, 2, 3)
plt.plot(range(num_iterations), bestS2, color='red')
plt.title('S2 over Iterations')
plt.xlabel('Iteration')
plt.ylabel('S2')

# Plotting average of average best fitness, S1, and S2
plt.subplot(2, 2, 4)
plt.bar(['Average Best Fitness', 'Average S1', 'Average S2'], [solution, s1, s2], color=['blue', 'green', 'red'])
plt.title('Average of Averages')
plt.ylabel('Value')
